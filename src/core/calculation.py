        ...
        if isinstance(cavity_calculation_or_filename, CavityCalculation):
            ...
            with h5py.File(filename, "r") as file:
                ...
                for domain_index in range(self.number_of_domains):
                    self.domain_triangles.append(
                        np.array(calculation["domain_information/domain_triangles/%d" % domain_index]))
                ...
                for multicavity_index in range(self.number_of_multicavities):
                    self.multicavities.append(
                        set(np.array(calculation["cavity_information/multicavities/%d" % multicavity_index])))
                    self.multicavity_triangles.append(
                        np.array(calculation["cavity_information/multicavity_triangles/%d" % multicavity_index]))
                if "center_cavity_information" in calculation:
                    self.number_of_center_multicavities = int(
                        calculation["center_cavity_information/number_of_multicavities"].value)
                    self.center_multicavity_volumes = np.array(
                        calculation["center_cavity_information/multicavity_volumes"])
                    self.center_multicavity_surface_areas = np.array(
                        calculation["center_cavity_information/multicavity_surface_areas"])
                    ...
                    for multicavity_index in range(self.number_of_center_multicavities):
                        self.center_multicavities.append(set(
                            np.array(calculation["center_cavity_information/multicavities/%d" % multicavity_index])))
                        self.center_multicavity_triangles.append(np.array(
                            calculation["center_cavity_information/multicavity_triangles/%d" % multicavity_index]))
                else:
                    self.number_of_center_multicavities = None
                    self.center_multicavity_volumes = None
                    self.center_multicavity_surface_areas = None
        ...
        with h5py.File(filename, "a") as file:
            if not use_center_points:
                if 'frame{}'.format(frame_nr) in file:
                    if self.resolution not in [calc.attrs['resolution'] for calc in
                                               file['frame{}'.format(frame_nr)].values()]:
                        calculation_nr = 1
                ...
                calculation_nr = 1
                while file['frame{}/calculation{}'.format(frame_nr, calculation_nr)].attrs[
            ...
            if not use_center_points:
                ...
                for domain_index in range(self.number_of_domains):
                    calculation["domain_information/domain_triangles/%d" % domain_index] = self.domain_triangles[
                        domain_index]
                ...
                for multicavity_index in range(self.number_of_multicavities):
                    calculation["cavity_information/multicavities/%d" % multicavity_index] = np.array(
                        list(self.multicavities[multicavity_index]))
                    calculation["cavity_information/multicavity_triangles/%d" % multicavity_index] = \
                        self.multicavity_triangles[multicavity_index]
            ...
            if self.number_of_center_multicavities is not None:
                ...
                calculation[
                    "center_cavity_information/multicavity_surface_areas"] = self.center_multicavity_surface_areas
                for multicavity_index in range(self.number_of_center_multicavities):
                    calculation["center_cavity_information/multicavities/%d" % multicavity_index] = np.array(
                        list(self.center_multicavities[multicavity_index]))
                    calculation["center_cavity_information/multicavity_triangles/%d" % multicavity_index] = \
